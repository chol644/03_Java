...
public class OrderAnalysis {
    public static void main(String[] args) {
        ...
        // 결과: {김철수=1850000.0, 이영희=825000.0, 박민수=530000.0, 최지원=2025000.0}3
        Map<String, Double> answer2 = orders.stream()
                .collect(Collectors.groupingBy(
                        //order -> order.getCustomerName(), // key 값
                        //Collectors.summingDouble(order->order.getTotalAmount()) // Value 값
        ...
        String answer3 =
                orders.stream()
                        .flatMap(order -> order.getProducts().stream()) // 주문 5개-> 1개로
                        .collect(Collectors.groupingBy(
        ...
        List<String> answer4 =
                orders.stream()
                        .filter(order -> order.getOrderDate().isAfter(LocalDate.of(2024, 4, 30)))
        ...
        Map<String, Double> answer5 =
                orders.stream()
                        .flatMap(order -> order.getProducts().stream()) // 상품스트림 생성
                        .collect(Collectors.groupingBy(
